...
NTSTATUS intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		...
		kdmLog(L"[-] Try restarting your computer to fix this." << std::endl);
		kdmLog(L"[-] You can also try to manually remove the driver from %temp%." << std::endl);
...
NTSTATUS intel_driver::Unload() {
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		...
		kdmLog(L"[+] Driver data destroyed\n" << std::endl);
	}
	file_ofstream.close();
	delete[] randomData;
