...
/*
 * MDL state for HVCI-compatible executable stub.
 * On Win11 25H2 with HVCI, NonPagedPoolExecute is blocked.
 * We allocate writable pool, write shellcode, then remap
 * a second mapping as PAGE_EXECUTE_READ via MDL.
 */
static PVOID g_SpoofStubBacking = NULL;  /* writable backing allocation */
static PMDL  g_SpoofStubMdl    = NULL;  /* MDL for executable remap    */
...
static BOOLEAN InitSpoofStub()
{
    ...
    /*
     * Strategy 1 (HVCI-safe): Allocate writable pool → write shellcode →
     * remap as executable via MDL.  Works on Win11 25H2 with HVCI.
     */
    g_SpoofStubBacking = ExAllocatePoolWithTag(NonPagedPool, sizeof(g_SpoofCode), 'pfSK');
    if (g_SpoofStubBacking) {
        RtlCopyMemory(g_SpoofStubBacking, g_SpoofCode, sizeof(g_SpoofCode));
        *(PVOID*)((PUCHAR)g_SpoofStubBacking + 25) = g_SpoofGadget;

        PMDL mdl = IoAllocateMdl(g_SpoofStubBacking, sizeof(g_SpoofCode),
            FALSE, FALSE, NULL);
        if (mdl) {
            /*
             * MmProbeAndLockPages on NonPagedPool is guaranteed safe —
             * the pages are always resident and valid.
             * DO NOT wrap in __try/__except: the driver is manually mapped
             * without .pdata registration, so SEH won't work during init.
             */
...

            {
                ...
                if (mapped) {
                    NTSTATUS st = MmProtectMdlSystemAddress(mdl, PAGE_EXECUTE_READ);
                    if (NT_SUCCESS(st)) {
                        g_SpoofStubMdl = mdl;
                        g_SpoofStub = mapped;
                        return TRUE;
        ...
        /* MDL approach failed — free backing, try legacy */
        ExFreePoolWithTag(g_SpoofStubBacking, 'pfSK');
        g_SpoofStubBacking = NULL;
    }

    /*
     * Strategy 2 (legacy): Direct NonPagedPoolExecute allocation.
     * Works on Win10 and Win11 without strict HVCI.
     * Returns NULL under HVCI — driver continues without spoof.
     */
...
{
    if (g_SpoofStubMdl) {
        /* HVCI path: unmap executable view, unlock, free MDL + backing */
        if (g_SpoofStub) {
            MmUnmapLockedPages(g_SpoofStub, g_SpoofStubMdl);
            g_SpoofStub = NULL;
        }
        MmUnlockPages(g_SpoofStubMdl);
        IoFreeMdl(g_SpoofStubMdl);
        g_SpoofStubMdl = NULL;
        ...
        /* Legacy path: direct NonPagedPoolExecute allocation */
