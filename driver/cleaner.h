...
static BOOLEAN CleanMmUnloadedDrivers()
{
    ...
    UNICODE_STRING targetName = RTL_CONSTANT_STRING(L"iqvw64e.sys");

    __try {
        for (int i = 0; i < 50; i++) {  /* Max 50 entries */
            if (drivers[i].Name.Buffer == NULL || drivers[i].Name.Length == 0)
                continue;

            /* Safe comparison using UNICODE_STRING (length-bounded, no null-term required) */
            if (RtlEqualUnicodeString(&drivers[i].Name, &targetName, TRUE)) {
        ...
        /* Structure may be paged or corrupted — bail safely */
    }
