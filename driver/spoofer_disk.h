...
/* ── Resolve IoDriverObjectType at runtime ───────────────────────── */

static POBJECT_TYPE* ResolveIoDriverObjectType()
{
    static POBJECT_TYPE* cached = NULL;
    if (cached) return cached;

    PVOID result = GetSystemModuleExport("ntoskrnl.exe", "IoDriverObjectType");
    if (!result || !MmIsAddressValid(result))
        return NULL;

    /* IoDriverObjectType is a POBJECT_TYPE* — verify the pointer it holds */
    POBJECT_TYPE* pType = (POBJECT_TYPE*)result;
    if (!MmIsAddressValid(pType) || !MmIsAddressValid(*pType))
        return NULL;
...
static BOOLEAN HookDiskDriver(PUNICODE_STRING driverName)
{
    ...
    /* Resolve the object type dynamically */
    POBJECT_TYPE* pDriverType = ResolveIoDriverObjectType();
    if (!pDriverType || !*pDriverType)
        return FALSE;
