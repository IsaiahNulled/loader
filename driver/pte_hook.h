...
/* TLB flush callbacks — must run on ALL cores after PTE/PDE changes */
static ULONG_PTR FlushEntireTlbIpi(ULONG_PTR) {
    __writecr3(__readcr3());  /* flush all non-global TLB entries */
    return 0;
}
...
static BOOLEAN InstallPteHook(PVOID targetFunction, PVOID handlerAddr)
{
    ...
    if (pdeEntry.LargePage) {
        ...
        /* Flush TLB on ALL CPUs — critical for multi-core stability.
         * Without this, other cores keep the stale 2MB TLB entry
         * and may access the wrong physical page → random BSOD. */
    ...
    /* Flush this single page from TLB on ALL CPUs */
