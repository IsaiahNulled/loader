...
static int update_window_manual_resize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_hovered, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4], const ImRect& visibility_rect)
{
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        if (nav_resize_dir.x != 0.0f || nav_resize_dir.y != 0.0f)
        {
            ...
            // g.NavDisableMouseHover = true; // Disabled for compatibility
...
{
    // Feature disabled: AddShadowRect requires custom ImGui
    ...
    // Use c_draw::shadow_rect if available, or skip drawing shadows.
    // For now, we skip.
    return;
}
...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v)
{
    ...
    draw->rect_filled(window->DrawList, bb.Min, bb.Max, draw->get_clr(clr->child, elements->scrollbar.alpha), s_(elements->scrollbar.rounding));
    draw->rect(window->DrawList, bb.Min, bb.Max, draw->get_clr(clr->border, elements->scrollbar.alpha), s_(elements->scrollbar.rounding));
    draw->rect_filled(window->DrawList, grab_rect.Min, grab_rect.Max, draw->get_clr(clr->accent, elements->scrollbar.alpha), s_(elements->scrollbar.rounding));
...
bool c_gui::begin(std::string_view name, bool* p_open, window_flags flags)
{
    ...
    // window_stack_data.StackSizesOnBegin.SetToContextState(&g); // Disabled for compatibility
