...
// Forward declaration — defined in anti_tamper.h
namespace antitamper { inline void RegisterThread(DWORD tid); }
...
inline void PostAuthHarden() {
    ErasePEHeader();
    // Re-snapshot after auth loaded networking DLLs (WinHTTP, etc.)
    g_moduleSnapshot.store(CountLoadedModules());
