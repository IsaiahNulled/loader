...
extern void CrashLog(const char* fmt, ...);
...
bool ResolveImports(portable_executable::vec_imports imports) {
	for (const auto& current_import : imports) {
		...
		for (auto& current_function_data : current_import.function_datas) {
			ULONG64 function_address = dell_driver::GetKernelModuleExport(Module, current_function_data.name);

			if (!function_address) {
				//Lets try with ntoskrnl
				if (Module != dell_driver::ntoskrnlAddr) {
					function_address = dell_driver::GetKernelModuleExport(dell_driver::ntoskrnlAddr, current_function_data.name);
					if (!function_address) {
#if !defined(DISABLE_OUTPUT)
...
ULONG64 mapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	CrashLog("MapDriver: allocating kernel memory, size=0x%X, mode=%d", image_size, (int)mode);
	ULONG64 kernel_image_base = 0;
	if (mode == AllocationMode::AllocateIndependentPages)
	{
		kernel_image_base = dell_driver::MmAllocateIndependentPagesEx(image_size);
	}
	else { // AllocatePool by default
		kernel_image_base = dell_driver::AllocatePool(nt::POOL_TYPE::NonPagedPool, image_size);
	}
	CrashLog("MapDriver: kernel_image_base=0x%llX", (unsigned long long)kernel_image_base);
	...
	do {
		...
		// Fix security cookie BEFORE relocating — SecurityCookie in LoadConfig is
		// still at its original image-relative value, so the local pointer arithmetic
		// is correct.  After RelocateImageByDelta the value becomes a kernel address
		// and the destroyHeader adjustment causes the subtraction to overshoot into
		// user-mode space, producing the 0x00007... RDX crash.
		CrashLog("MapDriver: fixing security cookie (pre-reloc)");
		if (!FixSecurityCookie(local_image_base, nt_headers->OptionalHeader.ImageBase))
		{
		...
		CrashLog("MapDriver: applying relocations (delta=0x%llX)", (unsigned long long)(kernel_image_base - nt_headers->OptionalHeader.ImageBase));
		RelocateImageByDelta(portable_executable::GetRelocs(local_image_base), kernel_image_base - nt_headers->OptionalHeader.ImageBase);

		CrashLog("MapDriver: resolving imports");
		...
		CrashLog("MapDriver: writing image to kernel at 0x%llX, size=0x%X", (unsigned long long)realBase, image_size);
		if (!dell_driver::WriteMemory(realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
		...
		if (mode == AllocationMode::AllocateIndependentPages)
		{
			...
			for (int i = 0; i < nt_headers->FileHeader.NumberOfSections; i++) {
				...
				if (!dell_driver::MmSetPageProtection(secAddr, secSize, prot)) {
		...
		CrashLog("MapDriver: calling DriverEntry at 0x%llX (param1=0x%llX param2=0x%llX)",
			(unsigned long long)address_of_entry_point, (unsigned long long)param1, (unsigned long long)param2);
		...
		CrashLog("MapDriver: about to CallKernelFunction for DriverEntry");
		NTSTATUS status = 0;
		if (!dell_driver::CallKernelFunction(&status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
		...
		CrashLog("MapDriver: DriverEntry returned 0x%X", (unsigned)status);
		...
		if (free) {
			...
			if (mode == AllocationMode::AllocateIndependentPages)
			{
				free_status = dell_driver::MmFreeIndependentPages(realBase, image_size);
			}
			else {
				free_status = dell_driver::FreePool(realBase);
			}
	...
	if (mode == AllocationMode::AllocateIndependentPages)
	{
		free_status = dell_driver::MmFreeIndependentPages(kernel_image_base, image_size);
	}
	else {
		free_status = dell_driver::FreePool(kernel_image_base);
	}
