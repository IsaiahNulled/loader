...
void c_draw::image_rounded(ImDrawList* draw_list, ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min, const ImVec2& uv_max, ImU32 col, float rounding, draw_flags flags)
{
    ...
    // Use standard AddImageRounded if available, or fallback to manual path
    // For now, assuming standard ImGui has AddImageRounded (added in 1.82)
...
{
    // Feature disabled: Requires custom ImGui fork with ShadowRectUvs
    return;
}
...
{
    // Feature disabled: Requires custom ImGui fork
    return;
}
