...
void RunAimbot() {
  ...
  // Silent aim: write body_rotation quaternion on PlayerEyes
  // This directly controls the server-side bullet direction without moving the camera
  if (Vars::Aim::silentAim && g_SDK && g_SDK->IsAttached()) {
    CLOG_CONTEXT("SilentAim::SetBodyRotation");
    uintptr_t localPlayer = g_SDK->GetLocalPlayer();
    if (localPlayer) {
      bool attackHeld = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) != 0;

      if (attackHeld) {
        // Write target rotation as quaternion to PlayerEyes.body_rotation
        Vec3 targetAngles;
        targetAngles.x = pitch;
        targetAngles.y = yaw;
        targetAngles.z = 0.0f;
        CLOG("SilentAim: writing bodyRot pitch=%.1f yaw=%.1f to 0x%llX", pitch, yaw, (unsigned long long)localPlayer);
        bool ok = g_SDK->SetBodyRotation(localPlayer, targetAngles);
        g_aimDbg.writeSuccess = ok;
      }
    }
  } else {
