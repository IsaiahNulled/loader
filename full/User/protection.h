...
static bool CheckTimingRDTSC() {
    ...
    // Do some work that a debugger single-step would make slow
    volatile int dummy = 0;
    for (int i = 0; i < 100; i++) dummy += i;
    ULONGLONG t2 = __rdtsc();
    // If someone is single-stepping, this gap will be huge
    return (t2 - t1) > 10000000; // ~10M cycles threshold
}
...
// Snapshot module count at startup, detect new modules being injected
...
// Check for known injection/debug tools loaded as DLLs
...
static void ProtectionThread() {
    ...
    while (g_protectionRunning.load()) {
        ...
        if (IsDebuggerDetected()) {
            // Silently crash or exit
            *((volatile int*)0) = 0; // instant crash, no clean exit for crackers
        }
        ...
        // Check for new DLLs injected
        uint32_t currentModules = CountLoadedModules();
        uint32_t baseline = g_moduleSnapshot.load();
        if (baseline > 0 && currentModules > baseline + 2) {
            // More than 2 new modules since startup = injection detected
            *((volatile int*)0) = 0;
        }
        ...
        // .text integrity check (skip if PE header was erased)
        uint32_t expectedCRC = g_textSectionCRC.load();
        if (expectedCRC != 0) {
            uint32_t currentCRC = ComputeTextSectionCRC();
            if (currentCRC != 0 && currentCRC != expectedCRC) {
                // Code was patched (breakpoints, hooks, NOP patches)
                *((volatile int*)0) = 0;
            }
        }

        Sleep(3000 + (GetTickCount() % 2000)); // Random interval 3-5s
    }
}

// ── Public API ──────────────────────────────────────────────────

// Call once at startup BEFORE any sensitive code
inline bool InitProtection() {
    // Hide main thread from debugger
    HideThreadFromDebugger();

    // Initial checks (fail fast)
    if (IsDebuggerDetected()) {
    ...
    if (CheckDebuggerWindows()) {
        printf(skCrypt("\n [!] Close debugging tools before running.\n"));
        return false;
    }
    ...
    // Snapshot module count baseline
    g_moduleSnapshot.store(CountLoadedModules());

    // Compute .text section CRC before any modifications
    g_textSectionCRC.store(ComputeTextSectionCRC());

    // Start background watchdog
    g_protectionRunning.store(true);
    std::thread watchdog(ProtectionThread);
    watchdog.detach();

    return true;
}

// Call after authentication is complete to make dumping harder
inline void PostAuthHarden() {
    // Erase PE header
    ErasePEHeader();

    // Re-snapshot modules (KeyAuth may have loaded networking DLLs)
    g_moduleSnapshot.store(CountLoadedModules());
}
