...
public:
    ...
    void SetBA(const Vec2& angles) {
        static int dbgSetBA = 0;
        if (!m_sdk || !IsValidPtr(m_addr)) {
            if (dbgSetBA++ % 600 == 0)
                printf("[AIMBOT] SetBA: invalid sdk/addr (addr=0x%llX)\n", (unsigned long long)m_addr);
            return;
        }
        std::uintptr_t playerInput = m_sdk->ReadVal<std::uintptr_t>(m_addr + Offsets::BasePlayer::playerInput);
        if (!IsValidPtr(playerInput)) {
            if (dbgSetBA++ % 600 == 0)
                printf("[AIMBOT] SetBA: playerInput invalid (local+0x%X = 0x%llX)\n",
                       Offsets::BasePlayer::playerInput, (unsigned long long)playerInput);
            return;
        }
        ...
        if (currentAngles.x < -90.f || currentAngles.x > 90.f ||
            currentAngles.y < -360.f || currentAngles.y > 360.f) {
            if (dbgSetBA++ % 600 == 0)
                printf("[AIMBOT] SetBA: sanity FAIL (angles=%.1f,%.1f,%.1f) - offset 0x%X wrong?\n",
        ...
        bool ok = m_sdk->WriteVal(playerInput + Offsets::PlayerInput::bodyAngles, newAngles);
        if (dbgSetBA++ % 600 == 0)
            printf("[AIMBOT] SetBA: write %s (input=0x%llX +0x%X pitch=%.1f yaw=%.1f)\n",
                   ok ? "OK" : "FAIL", (unsigned long long)playerInput,
                   Offsets::PlayerInput::bodyAngles, angles.x, angles.y);
    }
