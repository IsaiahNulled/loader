...
void SyncConfig() {
  ...
  Vars::Aim::multiBone = cfg->get<checkbox_t>("Multi-Bone Hit").enabled;
  Vars::Aim::multiBoneMode = cfg->get<dropdown_t>("Multi-Bone Mode").callback;
  ...
  g_removeLayers = cfg->get<checkbox_t>("Remove Layers").enabled;
  g_thirdPerson = cfg->get<checkbox_t>("Third Person").enabled;

  // Movement exploits
  g_spiderman = cfg->get<checkbox_t>("Spiderman").enabled;
  g_flyhack = cfg->get<checkbox_t>("Flyhack").enabled;
  g_omniSprint = cfg->get<checkbox_t>("Omni Sprint").enabled;

  // Terrain
  g_terrainRemover = cfg->get<checkbox_t>("Terrain Remover").enabled;
  ...
  g_fovChangerEnabled = cfg->get<checkbox_t>("FOV Changer").enabled;
  g_gameFov = cfg->get<slider_t>("Game FOV").callback;
  g_zoomFov = cfg->get<slider_t>("Zoom FOV").callback;

  // Zoom key mapping (same key list as aimbot)
  {
    static const int vkKeys[] = {
        VK_RBUTTON, VK_LBUTTON, VK_XBUTTON1, VK_XBUTTON2, VK_SHIFT,
        VK_CONTROL, VK_MENU,    0x43 /*C*/,  0x56 /*V*/,  0x58 /*X*/
    };
    int zki = cfg->get<dropdown_t>("Zoom Key").callback;
    if (zki >= 0 && zki < 10)
      g_zoomKey = vkKeys[zki];
    else
      g_zoomKey = VK_XBUTTON2;
  }
...
void SaveConfig(const std::string &path) {
  ...
  f << "removelayers=" << g_removeLayers << "\n";
  f << "thirdperson=" << g_thirdPerson << "\n";
  ...
  f << "fovchanger=" << g_fovChangerEnabled << "\n";
  f << "gamefov=" << g_gameFov << "\n";
  f << "zoomfov=" << g_zoomFov << "\n";
  ...
  f << "flyhack=" << g_flyhack << "\n";
  f << "omnisprint=" << g_omniSprint << "\n";
  f << "terrain=" << g_terrainRemover << "\n";
  ...
  while (std::getline(f, line)) {
    ...
    if (key == "chamsmatid") {
      ...
      const struct { unsigned int id; const char* name; } matNames[] = {
        {763998,"Solid White"},{518638,"Glow Red"},{232168,"Glow Green"},
        {102998,"Neon Purple"},{98400,"Green Glow"},{98648,"Red Glow"},
        {907390,"Emissive Light"},{1254166,"Glow"},{134056,"Blue Emissive"},
        {1364844,"Green Emissive"},{84446,"Laser Green"},{84438,"Laser Blue"},
        {1348630,"Holographic"}
      };
      bool valid = false;
      for (auto& m : matNames) if (m.id == loadedId) { g_chamsMaterialId = loadedId; g_chamsMaterialName = m.name; valid = true; break; }
      if (!valid) { g_chamsMaterialId = 763998; g_chamsMaterialName = "Solid White"; }
    }
    if (key == "vmchams")
    ...
    if (key == "vmchamsmatid") {
      ...
      if (!valid) { g_vmChamsMaterialId = 763998; g_vmChamsMaterialName = "Solid White"; }
    }
    if (key == "removelayers")
      g_removeLayers = (std::stoi(val) != 0);
    if (key == "thirdperson")
      g_thirdPerson = (std::stoi(val) != 0);
    ...
    if (key == "fovchanger")
      g_fovChangerEnabled = (std::stoi(val) != 0);
    if (key == "gamefov")
      g_gameFov = std::stof(val);
    if (key == "zoomfov")
      g_zoomFov = std::stof(val);

    // Movement exploits
    if (key == "spiderman")
      g_spiderman = (std::stoi(val) != 0);
    if (key == "flyhack")
      g_flyhack = (std::stoi(val) != 0);
    if (key == "omnisprint")
      g_omniSprint = (std::stoi(val) != 0);
    if (key == "terrain")
      g_terrainRemover = (std::stoi(val) != 0);
  }
  f.close();
