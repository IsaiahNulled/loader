...
public:
  ...
  void RuntimePopulateOffsets() {
    ...
    // PlayerWalkMovement
    resolve(offsets::PlayerWalkMovement::gravityMultiplier,
            "PlayerWalkMovement", "gravityMultiplier");
    resolve(offsets::PlayerWalkMovement::maxVelocity, "PlayerWalkMovement",
            "maxVelocity");
  ...
  bool RefreshEntityList() {
    ...
    entityCount = 0;
    dbgFrame++;
    entDbg = {}; // reset debug state

    bool verbose = dbgVerbose();
  ...
  bool IsPlayer(uintptr_t entity) {
    ...
    // Check if entity has BasePlayer fields like playerModel, eyes, inventory
  ...
  bool ReadPlayerBones(uintptr_t entity, PlayerData &out) {
    ...
    // Step 3: read each bone transform individually (simple, no batching)
    int validCount = 0;
    for (int i = 0; i < numWanted; i++) {
      ...
      uintptr_t transform =
          Read<uintptr_t>(boneArray + 0x20 + (uintptr_t)boneIdx * 8);
      if (!transform || !IsValidPtr(transform))
        continue;
  ...
  void WriteRecoilScaled(uintptr_t addr, float original, float reductionPct) {
    float scale = 1.0f - (reductionPct / 100.0f); // 100% reduction → 0 scale
    Write<float>(addr, original * scale);
  }
  ...
  bool ApplyNoRecoil(uintptr_t localPlayer) {
    ...
    if (!recoilPtr || !IsValidPtr(recoilPtr)) {
      ...
      if (dbgCount++ % 1800 == 0) {
        printf("[NR-DBG] recoilPtr FAIL: weapon=0x%llX +0x%X -> 0x%llX\n",
               (uint64_t)heldEntity, offsets::BaseProjectile::recoilProperties, (uint64_t)recoilPtr);
        // Wide scan: find ALL valid pointers on the BaseProjectile
        printf("[NR-DBG] === WIDE SCAN weapon 0x300-0x480 ===\n");
        for (int probe = 0x300; probe <= 0x480; probe += 8) {
          uintptr_t val = Read<uintptr_t>(heldEntity + probe);
          if (val && IsValidPtr(val)) {
            printf("[NR-DBG]   +0x%X = 0x%llX\n", probe, (uint64_t)val);
            // Dump floats 0x10-0x70 from this pointer to find recoil-like values
            bool hasNonZero = false;
            for (int fo = 0x10; fo <= 0x70; fo += 4) {
              float f = Read<float>(val + fo);
              if (f != 0.f && f > -100.f && f < 100.f) hasNonZero = true;
            }
            if (hasNonZero) {
              printf("[NR-DBG]     Floats: ");
              for (int fo = 0x10; fo <= 0x70; fo += 4) {
                float f = Read<float>(val + fo);
                if (f != 0.f && f > -100.f && f < 100.f)
                  printf("+0x%X=%.3f ", fo, f);
              }
              printf("\n");
            }
        ...
        // Also dump raw floats directly on the weapon itself (for aimCone etc.)
        printf("[NR-DBG] === DIRECT FLOATS on weapon 0x370-0x410 ===\n");
        for (int fo = 0x370; fo <= 0x410; fo += 4) {
          float f = Read<float>(heldEntity + fo);
          if (f != 0.f && f > -100.f && f < 100.f)
            printf("[NR-DBG]   weapon+0x%X = %.4f\n", fo, f);
        }
      }
      return false;
    }
    ...
    WriteRecoilScaled(recoilPtr + offsets::RecoilProperties::recoilYawMin,
                      cachedRecoil.yawMin, pct);
    WriteRecoilScaled(recoilPtr + offsets::RecoilProperties::recoilYawMax,
                      cachedRecoil.yawMax, pct);
    WriteRecoilScaled(recoilPtr + offsets::RecoilProperties::recoilPitchMin,
                      cachedRecoil.pitchMin, pct);
    WriteRecoilScaled(recoilPtr + offsets::RecoilProperties::recoilPitchMax,
                      cachedRecoil.pitchMax, pct);

    if (cachedRecoil.hasNew) {
      uintptr_t newRecoil =
          Read<uintptr_t>(recoilPtr + offsets::RecoilProperties::new_recoil);
      if (newRecoil && IsValidPtr(newRecoil)) {
        WriteRecoilScaled(newRecoil + offsets::RecoilProperties::recoilYawMin,
                          cachedRecoil.nYawMin, pct);
        WriteRecoilScaled(newRecoil + offsets::RecoilProperties::recoilYawMax,
                          cachedRecoil.nYawMax, pct);
        WriteRecoilScaled(newRecoil + offsets::RecoilProperties::recoilPitchMin,
                          cachedRecoil.nPitchMin, pct);
        WriteRecoilScaled(newRecoil + offsets::RecoilProperties::recoilPitchMax,
                          cachedRecoil.nPitchMax, pct);
      }
    }
  ...
  uintptr_t FindLocalPlayer() {
    ...
    if (camPos.x == 0.f && camPos.y == 0.f && camPos.z == 0.f) {
      printf("[FINDLOCAL] Camera position is zero, can't find local player\n");
      return 0;
    }
    ...
    printf("[FINDLOCAL] Checking %d entities, camera pos: (%.1f, %.1f, %.1f)\n", 
           count, camPos.x, camPos.y, camPos.z);

    for (int i = 0; i < count && i < 100; i++) {
      uintptr_t ent = GetEntity(i);
      
      // Debug: Show first few GetEntity results
      static int getEntityDebugCount = 0;
      if (getEntityDebugCount < 5) {
        printf("[GETENTITY] i=%d ent=0x%llX valid=%s\n", i, (uint64_t)ent, 
               (ent && IsValidPtr(ent)) ? "YES" : "NO");
        getEntityDebugCount++;
      }
      ...
      if (!pm) {
        printf("[FINDLOCAL] Player 0x%llX has no playerModel\n", (uint64_t)ent);
        continue;
      }
      ...
      printf("[FINDLOCAL] Player %d: 0x%llX pos:(%.1f,%.1f,%.1f) dist:%.2f\n", 
             playersChecked, (uint64_t)ent, pos.x, pos.y, pos.z, d);
      
      if (d < bestDist) {
        bestDist = d;
        best = ent;
        printf("[FINDLOCAL] New best: 0x%llX (dist %.2f)\n", (uint64_t)best, d);
      }
    }
    
    printf("[FINDLOCAL] Checked %d entities, %d players, best: 0x%llX\n", 
           entitiesChecked, playersChecked, (uint64_t)best);
    return best;
  }
  ...
  // Decrypt method 0: XOR, ADD, SHL1, SUB1 (encrypt_0x518)
  static float fovDec0(uint32_t enc) {
    uint32_t v = enc;
    v += 1;                    // reverse SUB 1
    v >>= 1;                   // reverse eax+eax (SHL 1)
  ...
  FovMethodEntry fovMethods[FOV_METHOD_COUNT] = {
    {"XOR+ADD+SHL1-1", fovDec0, fovEnc0},
  ...
  void ApplyFOV(float fovValue) {
    uintptr_t typeInfo = ResolveGraphicsTypeInfo();
    if (!typeInfo) {
      static int d1 = 0;
      if (d1++ % 600 == 0) printf("[FOV] Graphics TypeInfo not found\n");
      return;
    }
    uintptr_t sf = Read<uintptr_t>(typeInfo + offsets::Graphic::StaticFields);
    if (!sf || !IsValidPtr(sf) || sf < 0x10000 || sf > 0x7FFFFFFFFFFF) {
      static int d2 = 0;
      if (d2++ % 600 == 0) printf("[FOV] staticFields invalid: ti=0x%llX\n", (uint64_t)typeInfo);
      return;
    }
    ...
    // Determine where to read/write the encrypted value
    uintptr_t encAddr = 0;
    if (fovWrapperAddr != 0) {
      // Wrapper mode: re-read wrapper pointer each frame (GC can move objects)
      uintptr_t wrapper = Read<uintptr_t>(sf + fovFieldOff);
      if (wrapper && IsValidPtr(wrapper) && wrapper >= 0x10000 && wrapper <= 0x7FFFFFFFFFFF) {
        encAddr = wrapper + fovWrapperInnerOff;
        fovWrapperAddr = wrapper; // update cache
      } else {
        static int d3 = 0;
        if (d3++ % 600 == 0) printf("[FOV] Wrapper pointer invalid, re-calibrating\n");
        fovMethod = -1;
        return;
      }
    } else {
      // Direct mode: encrypted value is inline at sf+offset
      encAddr = sf + fovFieldOff;
    }

    // Read current value and log
    uint32_t curEnc = Read<uint32_t>(encAddr);
    float curFov = fovMethods[fovMethod].dec(curEnc);

    static int dbg = 0;
    if (dbg++ % 1800 == 0)
      printf("[FOV] method=%s %s+0x%X enc=0x%08X cur=%.1f target=%.1f\n",
             fovMethods[fovMethod].name,
             fovWrapperAddr ? "wrapper" : "sf",
             fovWrapperAddr ? fovWrapperInnerOff : fovFieldOff,
             curEnc, curFov, fovValue);

    // Sanity: if cur decrypt looks insane, re-calibrate
    if (curFov < 1.0f || curFov > 300.0f) {
      static int recal = 0;
      if (recal++ % 300 == 0) {
        printf("[FOV] Decrypt produced %.1f — re-calibrating\n", curFov);
        fovMethod = -1;
      }
      return;
    }

    // Safety: verify current value decrypts sanely before writing
    if (curFov < 30.0f || curFov > 180.0f) {
      fovMethod = -1;
      return;
    }

    // Verify wrapper hasn't been GC'd
    if (fovWrapperAddr != 0) {
      uintptr_t checkWrapper = Read<uintptr_t>(sf + fovFieldOff);
      if (checkWrapper != fovWrapperAddr) {
        fovMethod = -1;
        return;
      }
    }

    // Encrypt desired value and write
    uint32_t enc = fovMethods[fovMethod].enc(fovValue);
    Write<uint32_t>(encAddr, enc);
  }
  ...
  bool ValidateTodSky(uintptr_t candidate) {
    ...
    // Safety: reject obviously bad addresses (kernel space, null, etc.)
  ...
  int todReadAttempts = 0;  // Reset by ResolveTodSky before each cycle

  // Try a single candidate pointer as a TOD_Sky instance (direct or via GC handle)
  uintptr_t TryTodCandidate(uintptr_t raw) {
    if (!raw || raw < 0x10000 || raw > 0x7FFFFFFFFFFF) return 0;
    // Try direct
    if (ValidateTodSky(raw)) return raw;
    // Try as GC handle
    uintptr_t resolved = il2cpp_get_handle(raw);
    if (resolved && resolved != raw && resolved >= 0x10000 && resolved <= 0x7FFFFFFFFFFF) {
      if (ValidateTodSky(resolved)) return resolved;
    }
    return 0;
  }

  // Try resolving TOD_Sky from a TypeInfo pointer by scanning its static fields
  uintptr_t TryTodFromTypeInfo(uintptr_t typeInfo, bool verbose = false) {
    if (!typeInfo || !IsValidPtr(typeInfo)) return 0;
    if (typeInfo < 0x10000 || typeInfo > 0x7FFFFFFFFFFF) return 0;
    uintptr_t sf = Read<uintptr_t>(typeInfo + 0xB8);
    if (!sf || !IsValidPtr(sf)) return 0;
    if (sf < 0x10000 || sf > 0x7FFFFFFFFFFF) return 0;
    
    if (verbose)
      printf("[TOD]   typeInfo=0x%llX sf=0x%llX\n", (uint64_t)typeInfo, (uint64_t)sf);

    // Scan static fields for a TOD_Sky instance (direct or GC handle)
    // Check first ~32 pointer-sized slots: TOD_Sky.Instance is typically early
    for (int off = 0; off <= 0x100; off += 8) {
      todReadAttempts++;
      if (todReadAttempts > 50) return 0;  // Cap reads per TypeInfo
      uintptr_t candidate = Read<uintptr_t>(sf + off);
      if (!candidate || candidate < 0x10000 || candidate > 0x7FFFFFFFFFFF) continue;
      uintptr_t inst = TryTodCandidate(candidate);
      if (inst) {
        if (verbose)
          printf("[TOD]   FOUND at sf+0x%X: raw=0x%llX inst=0x%llX\n", off, (uint64_t)candidate, (uint64_t)inst);
        return inst;
      }
    }
    return 0;
  ...
  uintptr_t ResolveTodSky() {
    ...
    // Rate-limit: 5s normally, 15s after many failures
    ULONGLONG now = GetTickCount64();
    ULONGLONG cooldown = (failCount > 10) ? 15000 : 5000;
    if (now - lastAttempt < cooldown) return 0;
    lastAttempt = now;

    // Reset read attempt counter for this resolution cycle
    todReadAttempts = 0;
    bool verbose = (failCount % 5 == 0);  // Log every 5th attempt

    if (verbose)
      printf("[TOD] Resolving TOD_Sky (attempt %d, GA=0x%llX, RVA=0x%llX)...\n",
             failCount + 1, (uint64_t)gameAssembly, (uint64_t)offsets::tod_sky_pointer);

    // 1. Try primary pointer
    uintptr_t ti = Read<uintptr_t>(gameAssembly + offsets::tod_sky_pointer);
    if (verbose) {
      printf("[TOD]   GA+0x%llX = 0x%llX\n", (uint64_t)offsets::tod_sky_pointer, (uint64_t)ti);
      if (ti && IsValidPtr(ti)) {
        uintptr_t namePtr = Read<uintptr_t>(ti + 0x10);
        if (namePtr && IsValidPtr(namePtr)) {
          char nameBuf[32] = {};
          ReadRaw(namePtr, nameBuf, 31);
          nameBuf[31] = 0;
          printf("[TOD]   name='%s'\n", nameBuf);
        }
      }
    }
    uintptr_t inst = TryTodFromTypeInfo(ti, verbose);
    if (inst) {
      cachedInst = inst;
      failCount = 0;
      printf("[TOD] Resolved via primary pointer: 0x%llX\n", (uint64_t)inst);
      return inst;
    }

    // 2. Bulk scan: read chunks of GA .data section, extract TypeInfo pointers,
    //    and check each one's static fields for a TOD_Sky instance.
    //    Class names are obfuscated so we can't match by name.
    static bool bulkScanDone = false;
    if (!bulkScanDone) {
      bulkScanDone = true;
      if (verbose)
        printf("[TOD] Primary failed (ti=0x%llX), doing bulk .data scan...\n", (uint64_t)ti);
      
      const size_t CHUNK = 0x100000;  // 1MB
      uint8_t* chunk = new (std::nothrow) uint8_t[CHUNK];
      if (chunk) {
        // Scan the .data section where TypeInfo pointers live
        // Known working basenetworkable_pointer is at ~0xD7F0000
        uint64_t scanStart = 0xD000000;
        uint64_t scanEnd   = 0xE200000;
        int checked = 0;
        
        for (uint64_t scanOff = scanStart; scanOff < scanEnd; scanOff += CHUNK) {
          size_t rd = (size_t)min((uint64_t)CHUNK, scanEnd - scanOff);
          if (!ReadRaw(gameAssembly + scanOff, chunk, rd)) continue;
          
          for (size_t j = 0; j + 8 <= rd; j += 8) {
            uintptr_t candidate_ti = *(uintptr_t*)(chunk + j);
            if (!candidate_ti || candidate_ti < 0x10000 || candidate_ti > 0x7FFFFFFFFFFF) continue;
            if (candidate_ti == ti) continue; // Already tried
            
            // Quick filter: check if it looks like a TypeInfo by reading static_fields offset
            uintptr_t sf = Read<uintptr_t>(candidate_ti + 0xB8);
            if (!sf || sf < 0x10000 || sf > 0x7FFFFFFFFFFF) continue;
            
            checked++;
            if (checked > 2000) goto scan_done; // Safety cap on reads
            
            // Check if static fields contain a TOD_Sky instance
            todReadAttempts = 0;
            inst = TryTodFromTypeInfo(candidate_ti, false);
            if (inst) {
              uint64_t foundRVA = scanOff + j;
              cachedInst = inst;
              failCount = 0;
              printf("[TOD] FOUND via bulk scan at GA+0x%llX: inst=0x%llX (checked %d TypeInfos)\n",
                     foundRVA, (uint64_t)inst, checked);
              offsets::tod_sky_pointer = foundRVA;
              offsets::TodSky::Class = foundRVA;
              delete[] chunk;
              return inst;
            }
          }
        ...
        scan_done:
        delete[] chunk;
        printf("[TOD] Bulk scan done, checked %d TypeInfos, no TOD_Sky found\n", checked);
      }
    }

    failCount++;
    if (verbose)
      printf("[TOD] All scans failed (fail #%d)\n", failCount);
    return 0;
  }
  ...
  int VMProcessRenderer(uintptr_t renderer, unsigned int materialId) {
    ...
    for (uint32_t idx = 0; idx < 2; idx++) {
      ...
      uint64_t matSz = Read<uint64_t>(unityObj + 0x148 + 0x10);
      if (!matBase || !IsValidPtr(matBase) || matSz < 1 || matSz > 5) continue;
      for (uint32_t m = 0; m < (uint32_t)matSz; m++) {
        if (materialId == 999999) {
          // Custom Galaxy: patch colors directly on the material
          uintptr_t materialPtr = Read<uintptr_t>(matBase + (m * 0x4));
          if (materialPtr && IsValidPtr(materialPtr)) {
            // Try color property offsets
            for (int offset : {0x68, 0x70, 0x78, 0x80}) {
              uintptr_t colorPtr = materialPtr + offset;
              if (IsValidPtr(colorPtr)) {
                Write<float>(colorPtr + 0x0, 0.8f); // R (purple)
                Write<float>(colorPtr + 0x4, 0.2f); // G
                Write<float>(colorPtr + 0x8, 1.0f); // B (bright blue)
                Write<float>(colorPtr + 0xC, 1.0f); // A
                written++;
                break;
              }
            }
  ...
  int ApplyChamsCached(const std::vector<uintptr_t> &matAddrs,
    ...
    // Canary: read first address — valid material IDs are small (<100000)
    unsigned int canary = Read<unsigned int>(matAddrs[0]);
    if (canary > 100000 && canary != materialId)
  ...
  // Custom galaxy effect: patch material colors directly.
  // Single canary read, then pure writes. Returns 0 if stale.
  int ApplyCustomGalaxy(const std::vector<uintptr_t> &matAddrs) {
    if (matAddrs.empty()) return 0;

    // Canary: check first address is still a valid material ID
    unsigned int canary = Read<unsigned int>(matAddrs[0]);
    if (canary > 100000)
      return 0; // stale
    ...
    int applied = 0;
    for (const auto &addr : matAddrs) {
      uintptr_t materialBase = Read<uintptr_t>(addr - 0x148);
      if (!materialBase || !IsValidPtr(materialBase))
        continue;
      for (int offset : {0x68, 0x70, 0x78, 0x80}) {
        uintptr_t colorPtr = materialBase + offset;
        if (IsValidPtr(colorPtr)) {
          Write<float>(colorPtr + 0x0, 0.8f);
          Write<float>(colorPtr + 0x4, 0.2f);
          Write<float>(colorPtr + 0x8, 1.0f);
          Write<float>(colorPtr + 0xC, 1.0f);
          applied++;
          break;
        }
      }
...
public:
  ...
  bool InitializeTimeSystem() {
    ...
    if (!TodSkyTypeInfo) {
      printf("[TIME] Failed to get TOD_Sky TypeInfo\n");
      return false;
    }

    UINT64 Lists = Read<UINT64>(TodSkyTypeInfo + offsets::TodSky::static_fields);
    if (!Lists) {
      printf("[TIME] Failed to get TOD_Sky static fields\n");
      return false;
    }

    UINT64 List = Read<UINT64>(Lists + 0x18);
    if (!List) {
      printf("[TIME] Failed to get TOD_Sky list\n");
      return false;
    }

    UINT64 Instance = Read<UINT64>(List + 0x28);
    if (!Instance) {
      printf("[TIME] Failed to get TOD_Sky instance\n");
      return false;
    }
  ...
  bool SetTimeOfDay(float hour) {
    ...
    // Set the hour (0-24)
    Write<float>(Cycle + offsets::TodSky::Cycle::hour, hour);
    printf("[TIME] Set time to %.1f hour\n", hour);
    return true;
  }
  ...
  bool SetBrightness(float intensity) {
    ...
    printf("[TIME] Set brightness to %.2f\n", intensity);
    return true;
  }
  ...
  bool SetCustomFOV(float fov) {
    UINT64 typeInfo = Read<UINT64>(gameAssembly + offsets::Graphic::Base);
    if (!typeInfo) {
      printf("[FOV] Failed to get ConVar.Graphics TypeInfo\n");
      return false;
    }

    UINT64 staticFields = Read<UINT64>(typeInfo + offsets::Graphic::StaticFields);
    if (!staticFields) {
      printf("[FOV] Failed to get Graphics static fields\n");
      return false;
    }

    // Use the existing FOV system
    if (fovMethod == -1) {
      if (!CalibrateFOV(staticFields)) {
        printf("[FOV] Failed to calibrate FOV\n");
        return false;
      }
    }

    if (fovMethod >= 0 && fovMethod < FOV_METHOD_COUNT) {
      uint32_t enc = fovMethods[fovMethod].enc(fov);
      Write<uint32_t>(fovWrapperAddr + fovWrapperInnerOff, enc);
      printf("[FOV] Set FOV to %.1f (method: %s)\n", fov, fovMethods[fovMethod].name);
      return true;
    }
