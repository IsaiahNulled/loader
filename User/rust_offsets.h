...
namespace offsets {
    ...
    inline uint64_t tod_sky_pointer = 0xD78D028;  // TOD_Sky static class pointer (chain: +0xB8 +0xD0 +0x10 +0x20)
    ...
    namespace Graphic
    {
        inline int  fov = 0x518;  // Convar.Graphics._fov static field offset
    ...
    namespace TodSky
    {
        ...
        inline uint64_t  instance = 0x60;           // After static_fields
    ...
    namespace PlayerEyes {
        inline int body_rotation = 0x44;     // Quaternion (Vec4) - controls bullet direction
    ...
    namespace PlayerWalkMovement {
        inline int groundAngle = 0xC8;         // max walkable slope angle
        inline int groundAngleNew = 0xC4;      // new ground angle (float)
        inline int gravityMultiplier = 0x84;   // gravity multiplier
        inline int maxVelocity = 0x54;         // max velocity (float)
        inline int gravity = 0x7C;             // gravity value (for super jump)
        inline int spiderman = 0x108;          // slope override (set 0 = climb anything)
        inline int infiniteJump1 = 0xC0;       // jump cooldown (set 0 = can jump again)
        inline int infiniteJump2 = 0xBC;       // jump ground check (set large = always grounded)
    }
