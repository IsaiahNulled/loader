...
void c_gui::begin_group()
{
    ...
    // group_data.BackupActiveIdPreviousFrameIsAlive = g.ActiveIdPreviousFrameIsAlive; // Disabled
...
void c_gui::end_group()
{
    ...
    // const bool group_contains_prev_active_id = (group_data.BackupActiveIdPreviousFrameIsAlive == false) && (g.ActiveIdPreviousFrameIsAlive == true); // Disabled
...
void c_gui::end_def_child()
{
    ...
    // IM_ASSERT(g.WithinEndChild == false); // Disabled
    IM_ASSERT(child_window->Flags & window_flags_child_window);
    
    // g.WithinEndChild = true; // Disabled
    ...
    // g.WithinEndChild = false; // Disabled
    g.LogLinePosY = -FLT_MAX;
}
...
bool c_gui::button_behavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    ImGuiItemFlags item_flags = g.CurrentItemFlags; // Simplified
    ...
    // const ImGuiID test_owner_id = (flags & ImGuiButtonFlags_NoTestKeyOwner) ? ImGuiKeyOwner_Any : id; // Disabled
    const ImGuiID test_owner_id = id;
    if (hovered)
    {
        IM_ASSERT(id != 0);
        ...
        if (flags & (/*ImGuiButtonFlags_NoKeyModifiers*/ 0 | ImGuiButtonFlags_AllowOverlap) || (g.HoveredId == id && (g.HoveredIdTimer + 0.001f) < g.IO.KeyRepeatDelay))
        {
        ...
        // if (pressed)
        //    if (/*g.NavDisableHighlight == false && */ /*g.NavDisableMouseHover &&*/ g.NavWindow == window)
...
bool c_gui::item_hoverable(const ImRect& bb, ImGuiID id, ImGuiItemFlags item_flags)
{
    ...
    if (false /* g.NavDisableMouseHover */)
        return false;
...
void c_gui::set_style()
{
    ...
    style->window_padding = s_(0, 0);
    style->item_spacing = s_(0, 0);
    style->window_border_size = 0.f;
    style->window_shadow_offset = s_(0, 0);
    style->window_shadow_size = 0.f;
    style->scrollbar_border_padding = s_(elements->scrollbar.padding) + s_(3, 0);
    style->scrollbar_content_padding = s_(0);
    style->scrollbar_size = s_(elements->scrollbar.size);

    gui->easing(elements->scrollbar.size, var->gui.scrollbar_enabled ? 10.f : 1.f, 24.f, dynamic_easing);
    gui->easing(elements->scrollbar.padding.x, var->gui.scrollbar_enabled ? 5.f : -3.f, 24.f, dynamic_easing);
    gui->easing(elements->scrollbar.alpha, var->gui.scrollbar_enabled ? 1.f : 0.f, 6.f, static_easing);
}
...
void c_gui::draw_decorations()
{
    ...
    draw->rect(draw_list, rect.Min, rect.Max, draw->get_clr(clr->border), s_(elements->window.rounding));
}
