...
void FillPlayerCache(std::vector<PlayerData> &buffer) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if ((g_espSkeleton || Vars::Aim::enabled) && entity != localPlayer) {
...
void WorkerThreadRoutine() {
  ...
  while (g_Running) {
    if (g_SDK && g_SDK->IsAttached() && (g_espEnabled || Vars::Aim::enabled)) {
      ...
      // Update held entity for VM chams — FAST: Check every 500ms (was 3s) for quick weapon swap detection
      if (g_viewModelChams) {
        ...
        if (now - lastHeldCheck >= 500) {
          ...
          if (lp) {
            ...
            if (held != g_localHeldEntity) {
              // Weapon changed - force immediate VM cache rebuild
              g_localHeldEntity = held;
              {
                std::lock_guard<std::mutex> lk(g_chamsMutex);
                g_vmRenderers.clear(); // Force rebuild on next cycle
  ...
  for (const auto &player : g_cachedPlayers) {
    ...
    if (g_espDistance) {
      ...
      draw->AddText(ImVec2(tx + 1, screenFeet.y + 3), IM_COL32(0, 0, 0, 200),
                    buf);
      draw->AddText(ImVec2(tx, screenFeet.y + 2), IM_COL32(0, 255, 0, 255),
                    buf);
    }
    if (g_FontESP)
      ImGui::PopFont();
